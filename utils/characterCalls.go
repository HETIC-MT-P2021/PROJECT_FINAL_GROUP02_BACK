...
func GetCharacterById(id string) (*game.PlayerChar, error) {
	...
	switch err := charRow.Scan(&selectedCharacter.Id, &selectedCharacter.Name, &selectedCharacter.Player, &selectedCharacter.WeaponSkill, &selectedCharacter.Strength, &selectedCharacter.Endurance, &selectedCharacter.Agility, &selectedCharacter.Hitpoints, &selectedCharacter.IsCharAlive); err {
	case sql.ErrNoRows:
...
func GetBattleCharacterById(id string) (*game.CharBattle, error) {
	...
	err := charRow.Scan(&selectedCharacter.Id, &selectedCharacter.Name, &selectedCharacter.Player, &selectedCharacter.WeaponSkill, &selectedCharacter.Strength, &selectedCharacter.Endurance, &selectedCharacter.Agility, &selectedCharacter.Hitpoints, &selectedCharacter.IsFighting, &selectedCharacter.IsDodging, &selectedCharacter.IsFleeing)
	if err != nil {
